    ...
    <main className="min-h-screen bg-[#0D0D0D] text-white">
      <section className="border-b border-white/10 px-5 py-20 sm:px-8 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Car Detailing</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20">
        <div className="grid gap-6 lg:grid-cols-3">
          {pages.map((page) => (
            <a key={page.href} href={page.href} className="group rounded-[30px] border border-white/10 bg-[#111318] p-7 transition duration-300 hover:-translate-y-1 hover:border-[#5B8CFF]/35 hover:bg-[#5B8CFF]/[.035]">
              <p className="text-[11px] font-bold uppercase tracking-[.24em] text-[#5B8CFF]">{page.eyebrow}</p>
              ...
              <span className="mt-8 inline-block text-sm font-semibold text-white/70 group-hover:text-[#5B8CFF]">View pricing →</span>
            </a>
          ))}
        </div>
        <div className="mt-6 rounded-[28px] border border-white/10 bg-white/[.02] p-6 sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div><p className="text-xs font-bold uppercase tracking-[.25em] text-[#5B8CFF]">Marine Detailing</p><h2 className="mt-2 text-2xl font-semibold">Boat detailing has its own services and pricing.</h2></div>
      ...
      <section id="car-add-ons" className="scroll-mt-32 border-t border-white/10 bg-[#111318]/60">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 lg:grid-cols-[.72fr_1.28fr] lg:py-20">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.28em] text-[#5B8CFF]">Car Detailing Add-Ons</p>
            ...
            <div className="mt-6 flex flex-wrap gap-3"><a href="/quote" className="rounded-full bg-[#5B8CFF] px-5 py-3 text-sm font-semibold text-[#0D0D0D]">Get an Exact Quote</a><a href="/quote" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">Ask us about add-ons</a></div>
          </div>
          <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[#0D0D0D]">
            {carAddOns.map((item, index) => <div key={item.id} className={`flex items-center justify-between gap-5 px-5 py-4 sm:px-6 ${index !== carAddOns.length - 1 ? "border-b border-white/8" : ""}`}><span className="text-sm text-white/70">{item.name}</span><span className="text-sm font-semibold text-[#5B8CFF]">${item.price.toFixed(0)}</span></div>)}
          </div>
        </div>
      </section>
    </main>
