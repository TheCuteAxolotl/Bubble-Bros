...
export default async function FAQPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] text-white">
      <PublicHero content={content} imageCategory="faq-hero" eyebrowKey="faqEyebrow" titleKey="faqTitle" bodyKey="faqBody" action={<a href="/contact" className="inline-flex rounded-full bg-[#5B8CFF] px-6 py-3.5 text-sm font-semibold text-[#0D0D0D] hover:bg-[#5B8CFF]">Still need help?</a>} />
            ...
            <details key={index} className="group border-b border-white/8 last:border-b-0">
              <summary className="flex cursor-pointer items-center justify-between gap-6 px-6 py-6 sm:px-8 sm:py-7">
                <div className="flex items-start gap-4">
                  <span className="pt-1 text-[10px] font-semibold text-[#5B8CFF]">0{index + 1}</span>
