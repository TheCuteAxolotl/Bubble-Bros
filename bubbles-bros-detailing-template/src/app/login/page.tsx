    ...
    <div className="min-h-[72vh] bg-[#0D0D0D] px-6 py-16 text-white sm:py-24">
      <div className="mx-auto w-full max-w-md">
        <div className="rounded-[32px] border border-white/10 bg-[linear-gradient(145deg,rgba(74,85,104,.15),rgba(255,255,255,.02))] p-7 shadow-[0_28px_90px_rgba(0,0,0,.35)] sm:p-8">
          <p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#5B8CFF]">Customer Account</p>
          ...
          <form onSubmit={handleSubmit} className="mt-7 space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-white placeholder-white/25 outline-none transition focus:border-[#5B8CFF]/65 focus:bg-black/65 focus:shadow-[0_0_0_3px_rgba(255,45,45,.07)]" placeholder="your@email.com" required />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-white/72">Password</label>
              <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-white placeholder-white/25 outline-none transition focus:border-[#5B8CFF]/65 focus:bg-black/65 focus:shadow-[0_0_0_3px_rgba(255,45,45,.07)]" placeholder="••••••••" required />
            </div>
            <button type="submit" disabled={loading} className="w-full rounded-2xl bg-[#5B8CFF] py-3.5 font-semibold text-[#0D0D0D] shadow-[0_0_30px_rgba(255,45,45,.12)] hover:bg-[#5B8CFF] disabled:cursor-not-allowed disabled:opacity-45">
              {loading ? "Signing in..." : "Sign In"}
            </button>
          </form>

          <div className="mt-6 text-center text-sm text-white/42">
            Don&apos;t have an account? <a href={claimQuoteId ? `/register?claimQuoteId=${encodeURIComponent(claimQuoteId)}&email=${encodeURIComponent(email)}` : "/register"} className="font-semibold text-[#5B8CFF] hover:text-[#5B8CFF]">Create one</a>
          </div>
        </div>
      </div>
    </div>
