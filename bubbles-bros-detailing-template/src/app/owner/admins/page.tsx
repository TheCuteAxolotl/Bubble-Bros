...
const input = "w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none transition focus:border-[#5B8CFF]/60";
    ...
    <main className="min-h-screen bg-[#080808] text-white">
      <div className="mx-auto max-w-[1500px] px-5 py-9 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-5 border-b border-white/10 pb-7">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Owner only · Access control</p>
        ...
        {tab === "accounts" ? (
                  ...
                  {visibleAccounts.map((account) => (
                    <button key={account.id} onClick={() => setSelectedId(account.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selectedId === account.id ? "border-white/40 bg-white text-black" : "border-white/[.07] bg-black/20 hover:border-white/20"}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-bold uppercase ${selectedId === account.id ? "bg-black/8 text-black/55" : account.role === "owner" ? "bg-[#5B8CFF]/15 text-[#5B8CFF]" : account.role === "admin" ? "bg-sky-400/10 text-sky-200" : account.staffAccess ? "bg-violet-400/10 text-violet-200" : "bg-white/5 text-white/35"}`}>{account.role === "user" && account.staffAccess ? "staff" : account.role}</span>
                      </div>
                ...
                {selected ? (
                  <div className="space-y-7">
                    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/8 pb-6">
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#5B8CFF]">Account details</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {selected.latestBooking && <a href={`/booking-chat/${selected.latestBooking.id}`} className="rounded-full border border-white/12 px-4 py-2 text-xs font-semibold text-white/65 hover:text-white">Message via Bubbles & Bros.</a>}
                        {PHONE_FEATURE_ENABLED && selected.phone && <a href="/owner/calls" className="rounded-full border border-white/12 px-4 py-2 text-xs font-semibold text-white/65 hover:text-white">Business Phone</a>}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/8 pt-6">
                      <button onClick={saveAccount} disabled={saving} className="rounded-full bg-[#5B8CFF] px-6 py-3 text-sm font-bold text-[#0D0D0D] disabled:opacity-50">{saving ? "Saving…" : "Save account & access"}</button>
                      {!selected.protected && (
                        <div className="flex flex-wrap gap-2">
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-[380px_1fr]">
            <aside className="rounded-[26px] border border-white/10 bg-white/[.025] p-4">
              <button onClick={() => chooseRole(null)} className="w-full rounded-2xl border border-dashed border-[#5B8CFF]/30 bg-[#5B8CFF]/[.05] p-4 text-left text-sm font-semibold text-[#5B8CFF]">+ Create custom role</button>
            ...
            <form onSubmit={saveRole} className="rounded-[26px] border border-white/10 bg-white/[.025] p-6 sm:p-7">
              <p className="text-[10px] font-bold uppercase tracking-[.25em] text-[#5B8CFF]">{roleId ? "Edit custom role" : "New custom role"}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {ROLE_TEMPLATES.map((template) => (
                      <button
                        ...
                        className="rounded-full border border-white/10 bg-white/[.025] px-3.5 py-2 text-xs font-semibold text-white/55 transition hover:border-[#5B8CFF]/35 hover:text-white"
                      >
                        {template.name}
                      </button>
                    ))}
              ...
              <div className="mt-6"><p className="text-xs font-semibold uppercase tracking-[.18em] text-white/30">Role permissions</p><div className="mt-4 grid gap-3 md:grid-cols-2">{catalog.map((permission) => <label key={permission.key} className={`cursor-pointer rounded-2xl border p-4 transition ${rolePermissions.includes(permission.key) ? "border-[#5B8CFF]/35 bg-[#5B8CFF]/[.06]" : "border-white/[.07] bg-black/20"}`}><div className="flex gap-3"><input type="checkbox" className="mt-1" checked={rolePermissions.includes(permission.key)} onChange={(event) => setRolePermissions((current) => event.target.checked ? [...current, permission.key] : current.filter((key) => key !== permission.key))} /><div><p className="text-sm font-semibold">{permission.label}</p><p className="mt-1 text-xs leading-5 text-white/30">{permission.description}</p></div></div></label>)}</div></div>
              <div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-t border-white/8 pt-6"><button disabled={saving} className="rounded-full bg-[#5B8CFF] px-6 py-3 text-sm font-bold text-[#0D0D0D] disabled:opacity-50">{saving ? "Saving…" : roleId ? "Save role" : "Create role"}</button>{roleId && (() => { const current = roles.find((role) => role.id === roleId); return current ? <button type="button" onClick={() => deleteRole(current)} className="rounded-full border border-red-500/25 bg-red-500/[.06] px-5 py-3 text-sm font-semibold text-red-300">Delete role</button> : null; })()}</div>
            </form>
          </div>
        )}
      </div>
