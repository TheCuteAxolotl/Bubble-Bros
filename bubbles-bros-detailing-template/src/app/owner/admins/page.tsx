...
const input = "w-full rounded-2xl border border-[#000B3D]/10 bg-[#F4F7FB] px-4 py-3 text-sm text-[#0B0F19] outline-none transition focus:border-[#000B3D]/60";
...
export default function StaffAccountsPage() {
  ...
  if (loading) return <main className="min-h-screen bg-[#F7F9FC] text-[#0B0F19] grid place-items-center">Loading accounts…</main>;

  return (
    <main className="min-h-screen bg-[#F7F9FC] text-[#0B0F19]">
      <div className="mx-auto max-w-[1500px] px-5 py-9 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-5 border-b border-[#000B3D]/10 pb-7">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[.3em] text-[#000B3D]">Owner only · Access control</p>
            <h1 className="mt-2 text-4xl font-semibold tracking-[-.045em]">Staff & Accounts</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-black/45">View every account, promote or remove Admin access, create custom staff roles, assign dashboard permissions, contact customers, edit account information, or remove accounts.</p>
          </div>
          <a href="/owner/dashboard" className="rounded-full border border-[#000B3D]/15 px-5 py-2.5 text-sm font-semibold text-black/70 hover:text-[#0B0F19]">Back to dashboard</a>
        </div>

        <div className="mt-6 flex flex-wrap gap-2">
          <button onClick={() => setTab("accounts")} className={`rounded-full px-5 py-2.5 text-sm font-semibold ${tab === "accounts" ? "bg-white text-black" : "border border-[#000B3D]/10 text-black/60"}`}>Accounts</button>
          <button onClick={() => setTab("roles")} className={`rounded-full px-5 py-2.5 text-sm font-semibold ${tab === "roles" ? "bg-white text-black" : "border border-[#000B3D]/10 text-black/60"}`}>Custom Roles</button>
        </div>

        {message && <p className="mt-5 rounded-2xl border border-[#000B3D]/10 bg-[#F5F7FB] px-4 py-3 text-sm text-black/70">{message}</p>}
        ...
        {tab === "accounts" ? (
          <>
            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[["All accounts", counts.total], ["Admins", counts.admins], ["Custom-role staff", counts.customStaff], ["Customers", counts.customers]].map(([label, value]) => (
                <div key={String(label)} className="rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] p-4"><p className="text-xs text-black/40">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>
              ))}
            </div>

            <div className="mt-6 grid gap-5 xl:grid-cols-[380px_1fr]">
              <aside className="rounded-[26px] border border-[#000B3D]/10 bg-[#F7F9FC] p-4">
                ...
                <div className="mt-4 max-h-[900px] space-y-2 overflow-y-auto pr-1">
                  {visibleAccounts.map((account) => (
                    <button key={account.id} onClick={() => setSelectedId(account.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selectedId === account.id ? "border-[#000B3D]/40 bg-white text-black" : "border-[#000B3D]/10 bg-[#F7F9FC] hover:border-[#000B3D]/20"}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0"><p className="truncate font-semibold">{account.name}</p><p className={`mt-1 truncate text-xs ${selectedId === account.id ? "text-black/50" : "text-black/40"}`}>{account.email}</p></div>
                        <span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-bold uppercase ${selectedId === account.id ? "bg-black/8 text-black/55" : account.role === "owner" ? "bg-[#000B3D]/15 text-[#000B3D]" : account.role === "admin" ? "bg-sky-400/10 text-sky-700" : account.staffAccess ? "bg-violet-400/10 text-violet-700" : "bg-[#F4F7FB] text-black/40"}`}>{account.role === "user" && account.staffAccess ? "staff" : account.role}</span>
                      </div>
                      {account.customRoles.length > 0 && <p className={`mt-2 text-[10px] ${selectedId === account.id ? "text-black/45" : "text-black/35"}`}>{account.customRoles.map((role) => role.name).join(" · ")}</p>}
                    </button>
                  ))}
                </div>
              </aside>

              <section className="rounded-[26px] border border-[#000B3D]/10 bg-[#F7F9FC] p-5 sm:p-7">
                {selected ? (
                  <div className="space-y-7">
                    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#000B3D]/10 pb-6">
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#000B3D]">Account details</p>
                        <h2 className="mt-2 text-3xl font-semibold">{selected.name}</h2>
                        <p className="mt-1 text-sm text-black/40">Created {new Date(selected.createdAt).toLocaleDateString()} · Updated {new Date(selected.updatedAt).toLocaleDateString()}</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <a href={`mailto:${selected.email}`} className="rounded-full border border-[#000B3D]/10 px-4 py-2 text-xs font-semibold text-black/70 hover:text-[#0B0F19]">Email</a>
                        {selected.latestBooking && <a href={`/booking-chat/${selected.latestBooking.id}`} className="rounded-full border border-[#000B3D]/10 px-4 py-2 text-xs font-semibold text-black/70 hover:text-[#0B0F19]">Message via Bubbles & Bros.</a>}
                        {PHONE_FEATURE_ENABLED && selected.phone && <a href="/owner/calls" className="rounded-full border border-[#000B3D]/10 px-4 py-2 text-xs font-semibold text-black/70 hover:text-[#0B0F19]">Business Phone</a>}
                      </div>
                    </div>
                    ...
                    {selected.latestBooking && (
                      <div className="rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] p-4 text-sm text-black/60">
                        <p className="text-xs font-semibold uppercase tracking-[.16em] text-black/35">Latest booking/contact</p>
                        ...
                        <p className="mt-1 text-black/40">{selected.latestBooking.customerPhone}</p>
                      </div>
                    )}

                    {(selected.vehicles.length > 0 || selected.recentBookings.length > 0) && (
                      <div className="grid gap-4 lg:grid-cols-2">
                        <div className="rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[.16em] text-black/35">Saved vehicles</p>
                          <div className="mt-3 space-y-2">
                            {selected.vehicles.length ? selected.vehicles.map((vehicle) => (
                              <div key={vehicle.id} className="rounded-xl border border-[#000B3D]/[.08] bg-[#F7F9FC] px-3 py-2.5">
                                <p className="text-sm font-semibold">{vehicle.year} {vehicle.make} {vehicle.model}{vehicle.trim ? ` ${vehicle.trim}` : ""}</p>
                                <p className="mt-1 text-[11px] text-black/35">{[vehicle.nickname, vehicle.vehicleType, vehicle.color].filter(Boolean).join(" · ")}</p>
                              </div>
                            )) : <p className="text-xs text-black/35">No saved vehicles.</p>}
                          </div>
                        </div>
                        <div className="rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] p-4">
                          <p className="text-xs font-semibold uppercase tracking-[.16em] text-black/35">Recent bookings</p>
                          <div className="mt-3 space-y-2">
                            {selected.recentBookings.length ? selected.recentBookings.map((booking) => (
                              <a key={booking.id} href={`/booking-chat/${booking.id}`} className="block rounded-xl border border-[#000B3D]/[.08] bg-[#F7F9FC] px-3 py-2.5 transition hover:border-[#000B3D]/15">
                                <div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">{booking.serviceName}</p><span className="text-[10px] uppercase text-black/35">{booking.status}</span></div>
                                <p className="mt-1 text-[11px] text-black/35">{booking.vehicleYear} {booking.vehicleMake} {booking.vehicleModel}{booking.quotedPrice != null ? ` · $${booking.quotedPrice}` : ""}</p>
                              </a>
                            )) : <p className="text-xs text-black/35">No bookings yet.</p>}
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <label><span className="mb-2 block text-xs font-semibold uppercase tracking-[.15em] text-black/35">Name</span><input className={input} value={editName} onChange={(event) => setEditName(event.target.value)} disabled={selected.protected && false} /></label>
                      <label><span className="mb-2 block text-xs font-semibold uppercase tracking-[.15em] text-black/35">Email</span><input type="email" className={input} value={editEmail} onChange={(event) => setEditEmail(event.target.value)} disabled={selected.protected} /></label>
                      <label><span className="mb-2 block text-xs font-semibold uppercase tracking-[.15em] text-black/35">Base account role</span><select className={input} value={editRole} onChange={(event) => setEditRole(event.target.value)} disabled={selected.protected}><option value="user">Customer / standard account</option><option value="admin">Admin</option>{selected.protected && <option value="owner">Owner</option>}</select><span className="mt-2 block text-[11px] leading-5 text-black/30">Admin starts with Staff Guide, Bookings, Quote Chats, and Support. Individual access can still be customized below.</span></label>
                      <label><span className="mb-2 block text-xs font-semibold uppercase tracking-[.15em] text-black/35">Reset password</span><input type="password" minLength={8} className={input} value={editPassword} onChange={(event) => setEditPassword(event.target.value)} placeholder="Leave blank to keep current password" disabled={selected.protected} /></label>
                    </div>

                    {!selected.protected && (
                      <>
                        <div>
                          <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-black/35">Custom roles</p><h3 className="mt-2 text-xl font-semibold">Assigned roles</h3></div><button type="button" onClick={() => setTab("roles")} className="text-xs font-semibold text-[#000B3D]">Manage roles</button></div>
                          <div className="mt-4 grid gap-2 sm:grid-cols-2">
                            {roles.map((role) => (
                              <label key={role.id} className="flex cursor-pointer items-start gap-3 rounded-2xl border border-white/[.08] bg-[#F7F9FC] p-4"><input type="checkbox" className="mt-1" checked={editRoleIds.includes(role.id)} onChange={(event) => setEditRoleIds((current) => event.target.checked ? [...current, role.id] : current.filter((id) => id !== role.id))} /><span><span className="block text-sm font-semibold">{role.name}</span><span className="mt-1 block text-xs leading-5 text-black/35">{role.description || `${role.permissions.length} dashboard permission${role.permissions.length === 1 ? "" : "s"}`}</span></span></label>
                            ))}
                            {!roles.length && <p className="rounded-2xl border border-dashed border-[#000B3D]/10 p-5 text-sm text-black/35">No custom roles yet.</p>}
                          </div>
                        </div>

                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[.18em] text-black/35">Dashboard access</p>
                          <h3 className="mt-2 text-xl font-semibold">Per-account permissions</h3>
                          <p className="mt-2 text-sm leading-6 text-black/40">Inherit uses the Admin role and custom roles. Allow grants access directly. Deny removes access even when a role normally grants it.</p>
                          <div className="mt-4 space-y-2">
                            {catalog.map((permission) => {
                              const effective = selected.effectivePermissions.includes(permission.key);
                              return <div key={permission.key} className="grid gap-3 rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] p-4 sm:grid-cols-[1fr_150px] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold">{permission.label}</p><span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase ${effective ? "bg-emerald-400/10 text-emerald-700" : "bg-[#F4F7FB] text-black/35"}`}>{effective ? "Has access" : "No access"}</span></div><p className="mt-1 text-xs leading-5 text-black/35">{permission.description}</p></div><select className="rounded-xl border border-[#000B3D]/10 bg-white px-3 py-2 text-xs outline-none" value={editOverrides[permission.key] || "inherit"} onChange={(event) => setEditOverrides((current) => ({ ...current, [permission.key]: event.target.value as "inherit" | "allow" | "deny" }))}><option value="inherit">Inherit</option><option value="allow">Allow</option><option value="deny">Deny</option></select></div>;
                            })}
                          </div>
                        </div>
                      </>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#000B3D]/10 pt-6">
                      ...
                      {!selected.protected && (
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button onClick={deleteAccount} className="rounded-full border border-red-500/25 bg-red-500/[.06] px-5 py-3 text-sm font-semibold text-red-700">Delete account</button>
                        </div>
                      )}
                    </div>
                  </div>
                ) : <div className="grid min-h-[500px] place-items-center text-sm text-black/35">Choose an account.</div>}
              </section>
            </div>

            <form onSubmit={createAdmin} className="mt-6 rounded-[26px] border border-[#000B3D]/10 bg-[#F7F9FC] p-6">
              <p className="text-xs font-semibold uppercase tracking-[.18em] text-black/35">Quick create</p>
              <h2 className="mt-2 text-2xl font-semibold">Create a new Admin login</h2>
              <p className="mt-2 text-sm text-black/40">You can customize the new Admin’s access immediately after creating it.</p>
          ...
          <div className="mt-6 grid gap-5 xl:grid-cols-[380px_1fr]">
            <aside className="rounded-[26px] border border-[#000B3D]/10 bg-[#F7F9FC] p-4">
              <button onClick={() => chooseRole(null)} className="w-full rounded-2xl border border-dashed border-[#000B3D]/30 bg-[#000B3D]/[.05] p-4 text-left text-sm font-semibold text-[#000B3D]">+ Create custom role</button>
              <div className="mt-3 space-y-2">{roles.map((role) => <button key={role.id} onClick={() => chooseRole(role)} className={`w-full rounded-2xl border p-4 text-left ${roleId === role.id ? "border-[#000B3D]/40 bg-white text-black" : "border-[#000B3D]/10 bg-[#F7F9FC]"}`}><div className="flex justify-between gap-3"><span className="font-semibold">{role.name}</span><span className={`text-[10px] ${roleId === role.id ? "text-black/40" : "text-black/35"}`}>{role.assignedCount} assigned</span></div><p className={`mt-1 text-xs ${roleId === role.id ? "text-black/45" : "text-black/35"}`}>{role.permissions.length} permissions</p></button>)}</div>
            </aside>

            <form onSubmit={saveRole} className="rounded-[26px] border border-[#000B3D]/10 bg-[#F7F9FC] p-6 sm:p-7">
              <p className="text-[10px] font-bold uppercase tracking-[.25em] text-[#000B3D]">{roleId ? "Edit custom role" : "New custom role"}</p>
              <h2 className="mt-2 text-3xl font-semibold">{roleId ? roleName || "Custom role" : "Build a role"}</h2>
              <p className="mt-2 text-sm leading-6 text-black/40">Roles are reusable permission bundles. Assign a role to any account, then use per-account Deny/Allow overrides when that person needs an exception.</p>
              {!roleId && (
                <div className="mt-5">
                  <p className="text-[10px] font-bold uppercase tracking-[.2em] text-black/30">Start from a template</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {ROLE_TEMPLATES.map((template) => (
                      <button
                        ...
                        className="rounded-full border border-[#000B3D]/10 bg-[#F7F9FC] px-3.5 py-2 text-xs font-semibold text-black/60 transition hover:border-[#000B3D]/35 hover:text-[#0B0F19]"
                      >
                        {template.name}
                      </button>
                    ))}
