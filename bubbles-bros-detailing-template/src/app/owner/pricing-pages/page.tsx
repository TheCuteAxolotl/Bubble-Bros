...
const input = "mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white outline-none focus:border-[#5B8CFF]/55";
    ...
    <main className="min-h-screen bg-[#080808] text-white">
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        ...
        <div className="mt-8 flex flex-wrap gap-2 rounded-[22px] border border-white/10 bg-white/[.025] p-2">
          {(Object.keys(labels) as Kind[]).map((kind) => <button key={kind} onClick={() => setActive(kind)} className={`rounded-[16px] px-5 py-3 text-sm font-semibold transition ${active === kind ? "bg-[#5B8CFF] text-[#0D0D0D]" : "text-white/55 hover:bg-white/5 hover:text-white"}`}>{labels[kind]}</button>)}
        </div>
        ...
        <div className="mt-7 space-y-6">
          {config.packages.map((pkg, index) => (
            <article key={pkg.id} className="rounded-[28px] border border-white/10 bg-[#111318] p-5 sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div><p className="text-xs font-bold uppercase tracking-[.24em] text-[#5B8CFF]">Package {index + 1}</p><h3 className="mt-2 text-2xl font-semibold">{pkg.name}</h3><p className="mt-1 text-xs text-white/30">Photo placement: {labels[active]} → {pkg.name} → Package Photos</p></div>
        ...
        <button onClick={addPackage} className="mt-6 rounded-full border border-[#5B8CFF]/30 bg-[#5B8CFF]/8 px-5 py-3 text-sm font-semibold text-[#5B8CFF]">+ Add another package</button>

        <div className="sticky bottom-4 z-20 mt-10 flex flex-wrap items-center gap-4 rounded-[22px] border border-[#5B8CFF]/20 bg-[#0D0D0D]/95 p-4 shadow-2xl backdrop-blur-xl">
          <button onClick={save} disabled={saving} className="rounded-full bg-[#5B8CFF] px-7 py-3 font-semibold text-[#0D0D0D] disabled:opacity-50">{saving ? "Saving…" : "Save All Pricing Pages"}</button>
