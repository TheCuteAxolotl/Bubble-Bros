    ...
    <main className="min-h-screen bg-[#080808] text-white">
      <div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[.28em] text-[#5B8CFF]">Owner</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["open", "waiting", "closed"].map((status) => (
                      <button
                        ...
                        className={`rounded-full px-3 py-2 text-xs capitalize ${selected.status === status ? "bg-[#5B8CFF] text-[#0D0D0D]" : "border border-white/10 text-white/50"}`}
                      >
                        {status}
                      </button>
                    ))}
                    ...
                    {selected.status === "closed" && (
                      <button
                        ...
                        className="rounded-full border border-red-500/35 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300 transition hover:bg-red-500/20 hover:text-red-200"
                      >
                        Delete chat
                      </button>
                    )}
                ...
                <div className="space-y-3 py-6">
                  {selected.messages.map((supportMessage) => (
                    <div
                      ...
                      className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-6 ${supportMessage.sender === "team" ? "ml-auto bg-[#5B8CFF] text-[#0D0D0D]" : "bg-white/7 text-white/75"}`}
                    >
                      {supportMessage.body}
                    </div>
                  ))}
