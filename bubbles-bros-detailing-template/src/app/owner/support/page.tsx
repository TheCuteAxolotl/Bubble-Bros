...
export default function OwnerSupportPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data.user.role === "admin") return window.location.assign("/admin/support");
  ...
  const deleteClosedChat = async () => {
    if (!selected || selected.status !== "closed") return;

    const confirmed = window.confirm(
      `Delete the closed support chat “${selected.subject}”? This cannot be undone.`
    );
    ...
    if (!response.ok) {
      setMessage(data.error || "Could not delete support chat.");
      return;
    }
    ...
    setMessage(data.message || "Closed support chat deleted successfully.");
  };
    ...
    <main className="min-h-screen bg-[#F7F9FC] text-[#0B0F19]">
      <div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#000B3D]/10 pb-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[.28em] text-[#000B3D]">Owner</p>
            <h1 className="mt-2 text-4xl font-semibold tracking-[-.04em]">Support inbox</h1>
            <p className="mt-2 text-sm text-black/40">Reply to website support chats and manage repeat spam using the identifier from Discord.</p>
          </div>
          <a href="/owner/dashboard" className="rounded-full border border-[#000B3D]/15 px-5 py-2.5 text-sm text-black/70 transition hover:border-[#000B3D]/30 hover:text-[#0B0F19]">Back to dashboard</a>
        </div>
            ...
            {tickets.length ? (
              tickets.map((ticket) => (
                <button
                  ...
                  className={`w-full rounded-2xl p-4 text-left transition ${selectedId === ticket.id ? "bg-white text-black" : "hover:bg-[#F4F7FB]"}`}
                >
                  <div className="flex justify-between gap-3">
                    <span className="font-semibold">{ticket.name}</span>
                    <span className={`text-[10px] uppercase ${selectedId === ticket.id ? "text-black/45" : "text-black/35"}`}>{ticket.status}</span>
                  </div>
          ...
          <section className="min-h-[620px] rounded-[24px] border border-[#000B3D]/10 bg-[#F7F9FC] p-5 sm:p-7">
            {selected ? (
              <>
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#000B3D]/10 pb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selected.status === "closed" && (
                      <button
                        type="button"
                        onClick={deleteClosedChat}
                        className="rounded-full border border-red-500/35 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-700 transition hover:bg-red-500/20 hover:text-red-700"
                      >
                        Delete chat
                      </button>
                    )}
                ...
                <form onSubmit={sendReply} className="mt-auto flex gap-2 border-t border-[#000B3D]/10 pt-5">
                  <input value={reply} onChange={(event) => setReply(event.target.value)} placeholder="Reply to customer…" className="support-input" />
                  <button className="rounded-full bg-white px-5 text-sm font-semibold text-black transition hover:bg-[#F1F4F8]5">Send</button>
                </form>
              </>
            ) : (
        ...
        <section className="rounded-[24px] border border-red-500/20 bg-red-500/[.04] p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-black/40">Paste the IP address or visitor hash shown in the Discord support alert. The website does not display those identifiers anywhere.</p>
          ...
          <form onSubmit={block} className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr_auto_auto]">
            ...
            <button type="button" onClick={unblock} className="rounded-full border border-[#000B3D]/15 px-6 py-3 text-sm font-semibold text-black/70 transition hover:border-[#000B3D]/35 hover:bg-[#F4F7FB] hover:text-[#0B0F19]">Unblock</button>
          </form>

          {message && (
            <p className="mt-4 rounded-2xl border border-[#000B3D]/10 bg-[#F7F9FC] px-4 py-3 text-sm text-black/65">{message}</p>
          )}
        </section>
      </div>
    </main>
