...
const input = "w-full rounded-2xl border border-white/10 bg-black/50 px-4 py-3 outline-none transition focus:border-[#5B8CFF]/60 focus:shadow-[0_0_0_3px_rgba(255,45,45,.06)]";
...
export default function QuotePage() {
  ...
  if (!user) {
          ...
          <div className="mx-auto max-w-2xl rounded-[32px] border border-emerald-500/20 bg-emerald-500/[.05] p-7 sm:p-10">
            ...
            <div className="mt-8 rounded-2xl border border-white/10 bg-black/25 p-5">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <a href={`/register?claimQuoteId=${claim}&email=${email}&name=${name}`} className="rounded-full bg-[#5B8CFF] px-5 py-3 text-sm font-semibold text-[#0D0D0D]">Create account + save this quote</a>
    ...
    return (
      <main className="min-h-screen bg-[#0D0D0D] px-5 py-12 text-white sm:px-8 sm:py-20">
        <div className="mx-auto max-w-4xl">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[.28em] text-[#5B8CFF]">No login required</p>
          ...
          <form onSubmit={create} className="mt-10 rounded-[30px] border border-white/10 bg-white/[.025] p-5 sm:p-8">
            ...
            <div className="mt-5 grid gap-4">
              ...
              <label className="cursor-pointer rounded-2xl border border-dashed border-white/15 p-6 text-center text-sm text-white/45 hover:border-[#5B8CFF]/35">Add up to 3 photos so we can quote it better<input type="file" accept="image/*" multiple onChange={files} className="hidden" /></label>
              {photos.length > 0 && <p className="text-sm text-[#5B8CFF]">{photos.length} photo(s) ready</p>}

              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <p className="mt-3 pl-6 text-xs text-white/30">See our <a href="/privacy-policy" className="text-[#5B8CFF]">Privacy Policy</a> and <a href="/terms-and-conditions" className="text-[#5B8CFF]">Terms</a>.</p>
              </div>

              {error && <p className="rounded-2xl border border-red-500/25 bg-red-500/10 p-4 text-sm text-red-100">{error}</p>}
              <button disabled={submitting} className="rounded-full bg-[#5B8CFF] px-6 py-3.5 font-semibold text-[#0D0D0D] disabled:opacity-50">{submitting ? "Sending…" : "Send My Exact Quote Request"}</button>
  ...
  return (
    <main className="min-h-screen bg-[#0D0D0D] px-4 py-8 text-white">
      <div className="mx-auto max-w-7xl">
        <div>
          <p className="text-xs uppercase tracking-[.28em] text-[#5B8CFF]">Condition-based pricing</p>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[300px_1fr]">
          <aside className="rounded-[26px] border border-white/10 bg-white/[.025] p-3">
            <button onClick={() => setSelected("")} className="mb-3 w-full rounded-2xl bg-[#5B8CFF] px-4 py-3 text-left font-semibold text-[#0D0D0D]">+ New quote chat</button>
            ...
            {active ? (
              <div className="flex h-full flex-col">
                <div className="border-b border-white/10 pb-4">
                  <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-semibold">{active.subject}</h2><p className="mt-1 text-sm text-white/40">{active.vehicle ? `${active.vehicle.year} ${active.vehicle.make} ${active.vehicle.model}` : "No vehicle selected"} · {active.status}</p></div>{active.status === "accepted" && active.quotedPrice && <a href={`/contact?quote=${encodeURIComponent(active.id)}`} className="rounded-full bg-[#5B8CFF] px-5 py-2.5 text-sm font-semibold text-[#0D0D0D]">Book Now · ${active.quotedPrice.toFixed(2)}</a>}</div>
                  {active.quotedPrice && <div className="mt-4 rounded-2xl border border-[#5B8CFF]/20 bg-[#5B8CFF]/[.05] p-4"><p className="text-xs uppercase tracking-[.2em] text-[#5B8CFF]">Bubbles & Bros. Exact Quote</p><p className="mt-1 text-3xl font-semibold">${active.quotedPrice.toFixed(2)}</p>{active.quoteNotes && <p className="mt-2 text-sm text-white/45">{active.quoteNotes}</p>}{active.status === "quoted" && <button onClick={accept} className="mt-3 rounded-full bg-[#5B8CFF] px-5 py-2 text-sm font-semibold text-[#0D0D0D]">Accept quote</button>}{active.status === "accepted" && <div className="mt-3 flex flex-wrap items-center gap-3"><span className="text-sm font-semibold text-emerald-300">Quote accepted.</span><a href={`/contact?quote=${encodeURIComponent(active.id)}`} className="rounded-full bg-white px-5 py-2 text-sm font-semibold text-black">Book this quote</a></div>}{active.status === "booked" && <p className="mt-3 text-sm font-semibold text-emerald-300">Booking request submitted. This quote is now locked.</p>}</div>}
                  {active.status === "closed" && <div className="mt-4 rounded-2xl border border-white/10 bg-white/[.035] p-4 text-sm text-white/55">This conversation has been closed by Bubbles & Bros. You can still read the history, but new messages are disabled.</div>}
                </div>
                <div className="flex-1 space-y-3 overflow-y-auto py-5">{active.messages.map((message) => { if (message.sender === "system") return null; return <div key={message.id} className={`max-w-[82%] rounded-2xl px-4 py-3 ${message.sender === "customer" ? "ml-auto bg-white/10" : "bg-[#5B8CFF] text-[#0D0D0D]"}`}><p className="text-sm leading-6">{message.body}</p>{message.attachmentsJson && <div className="mt-3 grid grid-cols-2 gap-2">{JSON.parse(message.attachmentsJson).map((src:string,index:number)=><img key={index} src={src} alt="Customer upload" className="max-h-48 w-full rounded-xl object-cover" />)}</div>}</div>; })}<div ref={end} /></div>
                {readOnly ? <div className="border-t border-white/10 pt-4 text-sm text-white/38">{active.status === "booked" ? "This quote has been booked. Manage the appointment from your dashboard." : "This chat is closed and read-only."}</div> : <form onSubmit={send} className="border-t border-white/10 pt-4"><div className="flex gap-2"><input className={input} value={reply} onChange={(e)=>setReply(e.target.value)} placeholder="Message Bubbles & Bros.…" /><button className="rounded-full bg-[#5B8CFF] px-5 font-semibold text-[#0D0D0D]">Send</button></div><label className="mt-3 inline-block cursor-pointer text-xs text-white/45">Attach up to 3 photos<input type="file" accept="image/*" multiple onChange={files} className="hidden" /></label>{photos.length>0&&<span className="ml-3 text-xs text-[#5B8CFF]">{photos.length} photo(s) ready</span>}</form>}
                {error && <p className="mt-3 text-sm text-[#5B8CFF]">{error}</p>}
              </div>
            ) : (
              <form onSubmit={create} className="mx-auto max-w-2xl py-4">
                ...
                <div className="mt-5 grid gap-4">
                  ...
                  {!vehicles.length && <a href="/vehicles" className="text-sm text-[#5B8CFF]">+ Save a vehicle first</a>}
                  ...
                  {photos.length>0&&<p className="text-sm text-[#5B8CFF]">{photos.length} photo(s) selected</p>}
                  <div className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><label className="block text-sm text-white/60">Phone number (optional)<input type="tel" className={`${input} mt-2`} value={phone} onChange={(e)=>setPhone(e.target.value)} placeholder="(630) 555-0123" /></label></div>
                  {error&&<p className="text-sm text-[#5B8CFF]">{error}</p>}
                  <button disabled={submitting} className="rounded-full bg-[#5B8CFF] px-6 py-3 font-semibold text-[#0D0D0D] disabled:opacity-50">{submitting ? "Sending…" : "Request My Exact Quote"}</button>
                </div>
              </form>
            )}
          </section>
