...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!SMS_FEATURE_ENABLED || !user || !permissions.includes("smsInbox")) return;
    const loadUnread = () => {
  ...
  const cards = useMemo<Card[]>(() => [
    ...
    if (card.permission === "smsInbox" && !SMS_FEATURE_ENABLED) return false;
    if (card.permission === "businessPhone" && !PHONE_FEATURE_ENABLED) return false;
    return true;
  }), [smsUnread]);
