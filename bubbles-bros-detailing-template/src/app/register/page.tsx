...
export default function RegisterPage() {
  ...
  const inputClass = "w-full rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-white placeholder-white/25 outline-none transition focus:border-[#5B8CFF]/65 focus:bg-black/65 focus:shadow-[0_0_0_3px_rgba(255,45,45,.07)]";
    ...
    <div className="min-h-[72vh] bg-[#0D0D0D] px-6 py-16 text-white sm:py-24">
      <div className="mx-auto w-full max-w-md">
        <div className="rounded-[32px] border border-white/10 bg-[linear-gradient(145deg,rgba(74,85,104,.15),rgba(255,255,255,.02))] p-7 shadow-[0_28px_90px_rgba(0,0,0,.35)] sm:p-8">
          <p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#5B8CFF]">Bubbles & Bros. Account</p>
          ...
          <div className="mt-6 text-center text-sm text-white/42">Already have an account? <a href={claimQuoteId ? `/login?claimQuoteId=${encodeURIComponent(claimQuoteId)}&email=${encodeURIComponent(email)}` : "/login"} className="font-semibold text-[#5B8CFF] hover:text-[#5B8CFF]">Sign in</a></div>
        </div>
      </div>
    </div>
  );
