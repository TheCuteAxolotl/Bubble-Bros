  ...
  return (
    <main className="min-h-screen bg-[#0D0D0D] px-6 py-14 text-white">
      <div className="mx-auto max-w-5xl">
        <p className="text-xs font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Instant Estimate</p>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="rounded-[30px] border border-white/10 bg-[linear-gradient(145deg,rgba(74,85,104,.12),rgba(255,255,255,.025))] p-6">
            <div className="grid gap-5 md:grid-cols-2">
              <label className="text-sm text-white/55">Service
                <select value={serviceId} onChange={(event) => setServiceId(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 focus:border-[#5B8CFF]/60 focus:outline-none">
                  {services.map((service) => <option key={service.id} value={service.id}>{service.title}</option>)}
                </select>
              </label>
              <label className="text-sm text-white/55">Vehicle
                <select value={vehicle} onChange={(event) => setVehicle(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 focus:border-[#5B8CFF]/60 focus:outline-none">
              ...
              <label className="text-sm text-white/55">Overall condition
                <select value={condition} onChange={(event) => setCondition(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 focus:border-[#5B8CFF]/60 focus:outline-none">
              ...
              <label className="text-sm text-white/55">Paint / surface condition
                <select value={paint} onChange={(event) => setPaint(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 focus:border-[#5B8CFF]/60 focus:outline-none">
          ...
          <aside className="rounded-[30px] border border-[#5B8CFF]/20 bg-[#5B8CFF]/[.05] p-6">
            {result ? <>
              <p className="text-xs uppercase tracking-[.25em] text-[#5B8CFF]">Estimated range</p>
              ...
              <a href={`/quote?service=${result.service.id}`} className="mt-6 block rounded-full bg-[#5B8CFF] px-5 py-3 text-center font-semibold text-[#0D0D0D]">Get an Exact Quote</a>
