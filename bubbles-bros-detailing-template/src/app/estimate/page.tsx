  ...
  return (
    <main className="min-h-screen bg-white px-6 py-14 text-[#0B0F19]">
      <div className="mx-auto max-w-5xl">
        <p className="text-xs font-bold uppercase tracking-[.3em] text-[#000B3D]">Instant Estimate</p>
        <h1 className="mt-3 text-5xl font-semibold tracking-[-.05em]">Get a quick price range before you send a quote request.</h1>
        <p className="mt-4 max-w-2xl text-black/50">This is only an estimate. For an exact price, send us the vehicle info and a few photos.</p>

        <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="rounded-[30px] border border-[#000B3D]/10 bg-[linear-gradient(145deg,#F8FAFD,#EEF2FA)] p-6">
            <div className="grid gap-5 md:grid-cols-2">
              <label className="text-sm text-black/60">Service
                <select value={serviceId} onChange={(event) => setServiceId(event.target.value)} className="mt-2 w-full rounded-2xl border border-[#000B3D]/10 bg-white px-4 py-3 text-[#0B0F19] focus:border-[#000B3D]/60 focus:outline-none">
              ...
              <label className="text-sm text-black/60">Overall condition
                <select value={condition} onChange={(event) => setCondition(event.target.value)} className="mt-2 w-full rounded-2xl border border-[#000B3D]/10 bg-white px-4 py-3 text-[#0B0F19] focus:border-[#000B3D]/60 focus:outline-none">
          ...
          <aside className="rounded-[30px] border border-[#000B3D]/20 bg-[#000B3D]/[.05] p-6">
            {result ? <>
