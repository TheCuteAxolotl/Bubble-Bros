...
async function discordBooking(data: Record<string, string>) {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ content: lines.slice(0, 1950), username: "Bubbles & Bros." }),
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (quoteThreadId && auth?.id) {
      ...
      await prisma.quoteMessage.create({ data: { threadId: quoteThreadId, sender: "team", body: `Booking request submitted for $${bookingTotal.toFixed(2)}. Appointment confirmation will appear in your account once Bubbles & Bros. confirms it.` } });
    }
    ...
    if (smsConsent) {
      ...
      await sendTransactionalSms({
        to: phone,
        body: `Bubbles & Bros.: We received your booking request for ${serviceName} ($${bookingTotal.toFixed(2)})${requested ? `, requested for ${requested}` : ""}. We will notify you when it is confirmed. Reply to this text with questions or use your booking chat: ${bookingChatUrl}`,
      });
    }

    return NextResponse.json({ success: true, booking, chatUrl: bookingChatUrl, message: `Booking request submitted with a total of $${bookingTotal.toFixed(2)}. Bubbles & Bros. will confirm the appointment shortly.` }, { status: 201 });
  } catch (error) {
