...
function statusSms(booking: {
  ...
  if (booking.status === "confirmed") {
    return `Bubbles & Bros.: Your ${booking.serviceName} appointment is confirmed.${schedule}${total} Reply to this text with questions or updates. Booking chat: ${chatUrl}`;
  }
  if (booking.status === "cancelled") {
    return `Bubbles & Bros.: Your ${booking.serviceName} booking has been cancelled.${schedule} Reply to this text if you need help rescheduling. Booking chat: ${chatUrl}`;
  }
  if (booking.status === "completed") {
    return `Bubbles & Bros.: Your ${booking.serviceName} appointment is marked complete.${total} Thank you for choosing Bubbles & Bros. Reply to this text with any questions. Booking chat: ${chatUrl}`;
  }
  return null;
}
    ...
    if (wantsScheduleChange) {
      ...
      if (bookingHasSmsConsent(booking.notes)) {
        ...
        await sendTransactionalSms({
          ...
          body: `Bubbles & Bros.: Your ${booking.serviceName} appointment time was updated to ${when}. Reply to this text if you have any questions. Booking chat: ${chatUrl}`,
        });
      }
