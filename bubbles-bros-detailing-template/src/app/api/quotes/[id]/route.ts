  ...
  if (allowed.staff && !isRecentlyActive(allowed.thread.lastCustomerSeenAt)) {
    ...
    if (sms.consent && sms.phone) {
      ...
      await sendTransactionalSms({
        to: sms.phone,
        body: guestThread
          ? `Bubbles & Bros.: ${message || "We replied to your exact-quote request."} Reply to this text if you have a question.`.slice(0, 1450)
          : `Bubbles & Bros.: We replied to your quote chat. View your private conversation: ${getPublicSiteUrl()}/quote?thread=${encodeURIComponent(id)}`,
      });
    }
  }
    ...
    if (!isRecentlyActive(allowed.thread.lastCustomerSeenAt)) {
      ...
      if (sms.consent && sms.phone) {
        ...
        await sendTransactionalSms({
          to: sms.phone,
          body: guestThread
            ? `Bubbles & Bros.: Your exact quote is $${quotedPrice.toFixed(2)}${String(body.quoteNotes || "").trim() ? ` — ${String(body.quoteNotes).trim()}` : ""}. Reply to this text to book or ask a question. Want to track it online? ${getPublicSiteUrl()}/register?claimQuoteId=${encodeURIComponent(id)}&email=${encodeURIComponent(guestEmail)}`.slice(0, 1450)
            : `Bubbles & Bros.: Your final quote is $${quotedPrice.toFixed(2)}. Review or accept it here: ${getPublicSiteUrl()}/quote?thread=${encodeURIComponent(id)}`,
        });
      }
    }
