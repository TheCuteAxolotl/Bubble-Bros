...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!forwardToNumber) {
      response.say(
        { voice: "alice" },
        "Thanks for calling Bubbles & Bros. Our business phone is temporarily unavailable. Please send us a text and we will get back to you as soon as possible."
      );
      response.hangup();
      return xml(response);
    }
    ...
    response.say(
      { voice: "alice" },
      "Thanks for calling Bubbles & Bros. We cannot connect your call right now. Please send us a text and we will get back to you soon."
    );
    response.hangup();
