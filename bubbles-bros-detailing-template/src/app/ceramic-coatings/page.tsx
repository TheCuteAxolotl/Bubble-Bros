    ...
    <div className="min-h-screen bg-[#0D0D0D] text-white">
      <section className="relative isolate min-h-[620px] overflow-hidden border-b border-white/10">
        ...
        <div className="mx-auto flex min-h-[620px] max-w-[1540px] items-end border-x border-white/10 px-5 py-16 sm:px-8 lg:px-10 lg:py-20">
          <div className="max-w-4xl">
            <p className="text-[10px] font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Protection Guide</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3"><a href="/services" className="rounded-full bg-[#5B8CFF] px-6 py-3.5 text-sm font-semibold text-[#0D0D0D]">View ceramic coating</a><a href="/quote" className="rounded-full border border-white/15 bg-white/[.025] px-6 py-3.5 text-sm font-semibold">Ask us about your vehicle</a></div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1540px] border-x border-white/10 px-5 py-20 sm:px-8 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[.55fr_1.45fr]">
          <div><p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#5B8CFF]">Systems Bubbles & Bros. uses</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">We use GYEON and Gtechniq coating systems.</h2></div>
          <div className="grid gap-4 md:grid-cols-2">
            {systems.map((system) => <article key={system.name} className="rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,rgba(74,85,104,.15),rgba(255,255,255,.02))] p-7"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#5B8CFF]">{system.tag}</p><h3 className="mt-5 text-3xl font-semibold tracking-[-.04em]">{system.name}</h3><p className="mt-5 text-sm leading-7 text-white/48">{system.body}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-white/10 bg-[#111318]">
        <div className="mx-auto grid max-w-[1540px] gap-px border-x border-white/10 bg-white/10 md:grid-cols-4">
          ...
          ].map(([number, title, body]) => <div key={number} className="bg-[#111318] p-7 sm:p-8"><p className="text-xs font-semibold text-[#5B8CFF]">{number}</p><h3 className="mt-6 text-xl font-semibold">{title}</h3><p className="mt-3 text-sm leading-6 text-white/42">{body}</p></div>)}
        </div>
      </section>
      ...
      <section className="bg-white text-[#0D0D0D]">
        <div className="mx-auto grid max-w-[1540px] gap-10 border-x border-black/10 px-5 py-20 sm:px-8 lg:grid-cols-[1.15fr_.85fr] lg:px-10">
          <div><p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#5B8CFF]">What ceramic coating does</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.05em] sm:text-6xl">More protection, easier washing, and better water behavior.</h2></div>
