  ...
  return (
    <div className="min-h-screen bg-white text-[#0B0F19]">
      <section className="relative isolate min-h-[620px] overflow-hidden border-b border-[#000B3D]/10">
        <SitePhoto category="ceramic-coatings-hero" fallbackCategory="hero" className="absolute inset-0 -z-30 h-full w-full object-cover opacity-70" />
        ...
        <div className="mx-auto flex min-h-[620px] max-w-[1540px] items-end border-x border-[#000B3D]/10 px-5 py-16 sm:px-8 lg:px-10 lg:py-20">
          <div className="max-w-4xl">
            <p className="text-[10px] font-bold uppercase tracking-[.3em] text-[#000B3D]">Protection Guide</p>
            ...
            <p className="mt-6 max-w-2xl text-base leading-8 text-black/60 sm:text-lg">We use GYEON and Gtechniq coatings. Before any coating goes on, the paint gets cleaned, decontaminated, and corrected as needed.</p>
            <div className="mt-8 flex flex-wrap gap-3"><a href="/services" className="rounded-full bg-[#000B3D] px-6 py-3.5 text-sm font-semibold text-[#0B0F19]">View ceramic coating</a><a href="/quote" className="rounded-full border border-[#000B3D]/15 bg-[#F7F9FC] px-6 py-3.5 text-sm font-semibold">Ask us about your vehicle</a></div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1540px] border-x border-[#000B3D]/10 px-5 py-20 sm:px-8 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[.55fr_1.45fr]">
          <div><p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#000B3D]">Systems Bubbles & Bros. uses</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">We use GYEON and Gtechniq coating systems.</h2></div>
          <div className="grid gap-4 md:grid-cols-2">
            {systems.map((system) => <article key={system.name} className="rounded-[28px] border border-[#000B3D]/10 bg-[linear-gradient(145deg,#F8FAFD,#EEF2FA)] p-7"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#000B3D]">{system.tag}</p><h3 className="mt-5 text-3xl font-semibold tracking-[-.04em]">{system.name}</h3><p className="mt-5 text-sm leading-7 text-black/50">{system.body}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1540px] border-x border-[#000B3D]/10 px-5 py-14 sm:px-8 lg:px-10"><PricingMediaStrip category="ceramic-results" /></section>
