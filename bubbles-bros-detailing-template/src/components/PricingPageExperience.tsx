    ...
    <main className="min-h-screen bg-[#0D0D0D] text-white">
      <section className="relative isolate overflow-hidden border-b border-white/10">
        ...
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28">
          <p className="text-xs font-bold uppercase tracking-[.3em] text-[#5B8CFF]">{config.eyebrow}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#pricing" className="rounded-full bg-[#5B8CFF] px-6 py-3 text-sm font-semibold text-[#0D0D0D]">View Packages</a>
      ...
      <section id="pricing" className="mx-auto max-w-7xl scroll-mt-32 px-5 pb-20 sm:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <p className="text-xs font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Packages</p>
        ...
        <div className="mx-auto mt-9 flex max-w-3xl flex-wrap justify-center gap-2 rounded-[26px] border border-white/10 bg-[#111318] p-3">
          {(Object.keys(VEHICLE_LABELS) as VehicleClass[]).map((key) => (
            <button
              ...
              className={`min-w-[145px] flex-1 rounded-[20px] border px-5 py-4 text-sm font-semibold transition duration-300 ${vehicle === key ? "border-[#5B8CFF]/55 bg-[#5B8CFF]/10 text-white shadow-[0_0_32px_rgba(255,45,45,.10)]" : "border-white/10 bg-black/15 text-white/55 hover:border-[#5B8CFF]/30 hover:text-white"}`}
            >
              {VEHICLE_LABELS[key]}
            </button>
          ))}
        ...
        <div className={`mt-12 grid gap-6 ${packageCount >= 3 ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}>
          {config.packages.map((pkg) => {
            ...
            return (
              <article key={pkg.id} className={`relative flex min-h-full flex-col overflow-hidden rounded-[32px] border p-6 sm:p-7 ${pkg.featured ? "border-[#5B8CFF]/45 bg-[linear-gradient(180deg,rgba(255,45,45,.07),rgba(74,85,104,.08)_24%,rgba(255,255,255,.02))] shadow-[0_22px_70px_rgba(0,0,0,.32)]" : "border-white/10 bg-[#111318]"}`}>
                {pkg.badge && <span className="absolute right-5 top-5 rounded-full bg-[#5B8CFF] px-4 py-2 text-[10px] font-black uppercase tracking-[.18em] text-[#0D0D0D]">{pkg.badge}</span>}
                <PricingMediaStrip category={`pricing-${slug}-pkg-${pkg.id}`} className="mb-6" />
                <p className="text-[11px] font-bold uppercase tracking-[.24em] text-[#5B8CFF]">{pkg.tier}</p>
                ...
                <div className="mt-6 border-y border-white/10 py-5">
                  ...
                  <p className="mt-2 text-5xl font-semibold tracking-[-.05em]"><span className="mr-1 text-2xl text-[#5B8CFF]">$</span>{price.toFixed(0)}</p>
                </div>
                <ul className="mt-6 flex-1 space-y-0">
                  {pkg.features.map((feature) => (
                    <li key={feature} className="flex gap-3 border-b border-white/[.07] py-3 text-sm leading-6 text-white/68 last:border-b-0">
                      <span className="font-black text-[#5B8CFF]">✓</span><span>{feature}</span>
                    </li>
                  ))}
                </ul>
                <a href={bookingHref} className={`mt-7 block rounded-[20px] px-5 py-4 text-center text-sm font-bold transition duration-300 hover:-translate-y-0.5 ${pkg.featured ? "bg-[#5B8CFF] text-[#0D0D0D] shadow-[0_0_30px_rgba(255,45,45,.13)]" : "border border-white/15 bg-white/[.035] text-white hover:border-[#5B8CFF]/35 hover:bg-[#5B8CFF]/8"}`}>
                  {pkg.ctaLabel} →
                </a>
              </article>
            );
      ...
      <section className="border-y border-white/10 bg-[#111318]/70">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div><p className="text-xs font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Recent Work</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.05em]">See more of the work we’ve done recently.</h2></div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="rounded-[34px] border border-[#5B8CFF]/20 bg-[linear-gradient(130deg,rgba(255,45,45,.08),rgba(74,85,104,.08),rgba(255,255,255,.015))] p-8 sm:p-12">
          <p className="text-xs font-bold uppercase tracking-[.28em] text-[#5B8CFF]">Package price or exact quote</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3"><a href="#pricing" className="rounded-full bg-[#5B8CFF] px-6 py-3 font-semibold text-[#0D0D0D]">Choose a package</a><a href="/quote" className="rounded-full border border-white/15 px-6 py-3 font-semibold">Get an Exact Quote</a></div>
        </div>
      </section>
    </main>
  );
