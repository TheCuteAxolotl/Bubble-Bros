...
export default function RelatedServiceLinks({ currentPath }: { currentPath?: string }) {
  ...
  return (
    <section className="border-t border-[#000B3D]/10 bg-white px-5 py-14 text-[#0B0F19] sm:px-8 lg:py-20">
      <div className="mx-auto max-w-[1180px]">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.26em] text-[#000B3D]">Related services</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">Not sure which service fits your vehicle?</h2>
          </div>
          <a href="/quote" className="inline-flex w-fit rounded-full bg-[#000B3D] px-5 py-3 text-sm font-semibold text-[#0B0F19]">Get an Exact Quote</a>
        </div>
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((service) => (
            <a key={service.href} href={service.href} className="group rounded-[22px] border border-[#000B3D]/10 bg-[#F7F9FC] p-5 transition hover:border-[#000B3D]/20 hover:bg-[#F3F6FA]">
