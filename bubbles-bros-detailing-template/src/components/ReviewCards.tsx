...
function Stars({ rating }: { rating: number }) {
  ...
  return (
    <span className="tracking-[0.14em] text-[#5B8CFF]" aria-label={`${rating} out of 5 stars`}>
      {"★".repeat(rounded)}
...
export default function ReviewCards() {
  ...
  if (failed || !data) {
    return (
      <div className="rounded-[2rem] border border-[#5B8CFF]/20 bg-[#111318] p-8">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-5 inline-flex rounded-full bg-[#5B8CFF] px-5 py-2.5 text-sm font-black text-[#0D0D0D] transition hover:bg-[#5B8CFF]"
        >
          Open Google Reviews
        </a>
      </div>
  ...
  return (
    <section aria-label="Google reviews">
      <div className="mb-7 flex flex-col gap-5 rounded-[2rem] border border-white/10 bg-[#111318] p-7 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.28em] text-[#5B8CFF]">Live from Google Maps</p>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex rounded-full bg-[#5B8CFF] px-5 py-2.5 text-sm font-black text-[#0D0D0D] transition hover:bg-[#5B8CFF]"
          >
          ...
          <article key={review.id} className="flex h-full flex-col rounded-[2rem] border border-white/10 bg-[#111318] p-7">
            <div className="flex items-center gap-4">
              {review.authorPhotoUrl ? (
                ...
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#5B8CFF]/12 text-lg font-black text-[#5B8CFF]">
              ...
              <div className="min-w-0">
                {review.authorProfileUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="font-black text-white hover:text-[#5B8CFF]"
                  >
                    {review.author}
                  </a>
                ) : (
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-6 inline-flex w-fit text-sm font-black text-[#5B8CFF] transition hover:text-[#5B8CFF]"
            >
