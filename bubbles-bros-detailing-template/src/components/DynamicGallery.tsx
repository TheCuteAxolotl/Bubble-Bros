...
type Img = { id: string; url: string; title: string; category: string };
...
export default function DynamicGallery({ limit }: { limit?: number }) {
  ...
  useEffect(() => {
    ...
    ["gallery", "before-after", "portfolio"].forEach((category) =>
  ...
  const visible = useMemo(
    () => (typeof limit === "number" ? images.slice(0, limit) : images),
    [images, limit]
  );
      ...
      {visible.map((img, i) => (
        <article
          ...
          className={`group relative overflow-hidden rounded-[22px] ${
            i === 0 ? "sm:row-span-2 lg:col-span-2 lg:row-span-2" : ""
          }`}
        >
          <img
            src={img.url}
