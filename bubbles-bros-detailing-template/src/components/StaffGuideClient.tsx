    ...
    <main className="min-h-screen bg-[#070707] text-white">
      <header className="border-b border-white/10 bg-[radial-gradient(circle_at_20%_-20%,rgba(255,45,45,.18),transparent_38%),#080808]">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[.3em] text-[#5B8CFF]">Internal · Bubbles & Bros.</p>
              <h1 className="mt-3 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Staff Guide</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/45">Rules, communication standards, customer-service scripts, and the operating playbook for representing Bubbles & Bros. professionally.</p>
            </div>
          ...
          <div className="mt-7 max-w-xl">
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search rules, bookings, quotes, support…" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3.5 text-sm outline-none transition focus:border-[#5B8CFF]/60" />
          </div>
          {quickLinks.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-2">
              {quickLinks.map((item) => (
                <a key={item.label} href={item.href} className="rounded-full border border-white/10 bg-white/[.035] px-4 py-2 text-xs font-semibold text-white/55 transition hover:border-[#5B8CFF]/35 hover:text-white">
                  {item.label} ↗
                </a>
              ))}
            </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-10 sm:px-8 lg:grid-cols-[250px_1fr] lg:px-12">
        <aside className="h-fit rounded-[24px] border border-white/10 bg-white/[.025] p-4 lg:sticky lg:top-6">
          ...
          <div className="mt-5 rounded-2xl border border-[#5B8CFF]/20 bg-[#5B8CFF]/[.06] p-4 text-xs leading-5 text-white/55">
        ...
        <div className="space-y-6">
          {filtered.map((section) => (
            <section id={section.id} key={section.id} className="scroll-mt-6 rounded-[28px] border border-white/10 bg-white/[.025] p-6 sm:p-8">
              <p className="text-[10px] font-bold uppercase tracking-[.26em] text-[#5B8CFF]">{section.eyebrow}</p>
                ...
                <div className="mt-7 border-t border-white/8 pt-6">
                  ...
                  <div className="mt-4 grid gap-3 xl:grid-cols-2">
                    {section.scripts.map((script) => {
                      ...
                      return (
                        <div key={script.label} className="rounded-2xl border border-[#5B8CFF]/15 bg-[#5B8CFF]/[.045] p-5">
                          <div className="flex items-center justify-between gap-3">
                            <p className="text-xs font-bold uppercase tracking-[.16em] text-[#5B8CFF]">{script.label}</p>
                            <button
                              type="button"
                              onClick={async () => {
                                try {
