...
export async function notifyQuoteDiscord(data: QuoteDiscordPayload) {
  ...
  const lines = [
    ...
    "",
    `Open quote inbox: ${(process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000").replace(/\/$/, "")}/owner/quotes?thread=${encodeURIComponent(data.threadId)}`,
  ].filter(Boolean);
